	...
	(function(window, document, $, undefined) {
		...
		window.SaveHandler = new function() {
			...
			this.Encode = function(save, formatType, version) {
				...
				if (!formatType) {
					var formatKeys = Object.keys(formatTypeChecks);
					formatType = formatKeys[formatKeys.length - 1];
				}
				if (!version)
					version = 0;
			...
			var Format = new function() {
				...
				{ this.format = [																				// Description
					...
					{ 'format': 'Uint32', 'key': 'rngState', 'cond': 'saveVersion >= 4' }, 						// ??? Some global RNG state?
					...
					{ 'format': 'Float64', 'key': 'rubies', 'cond': 'saveVersion >= 4' },						// Current Rubies
					{ 'format': 'Float64', 'key': 'excavations', 'cond': 'saveVersion >= 4' },					// ??? Current Excavation count
					...
					{ 'format': 'Int8', 'key': 'empoweredTier', 'cond': 'saveVersion < 11' },					// Grand Balance target 1 building tier
					{ 'format': 'Int8', 'key': 'empoweredTier2', 'cond': [ 'saveVersion >= 5', 'saveVersion < 11' ] }, // Grand Balance target 2 building tier
					{ 'format': 'Float64', 'key': 'empoweredBonus', 'cond': 'saveVersion < 11' }, 				// Grand Balance target 1 multiplier
					{ 'format': 'Float64', 'key': 'empoweredBonus2', 'cond': [ 'saveVersion >= 5', 'saveVersion < 11' ] }, // Grand Balance target 2 multiplier
					{ 'format': 'Int32', 'key': 'miracleTier', 'cond': 'saveVersion >= 3' },					// Miracle target building tier
					{ 'format': 'Int32', 'key': 'miracleTimer', 'cond': 'saveVersion >= 3' },					// Miracle time left
					{ 'format': 'Int8', 'key': 'snowballScryUses', 'cond': 'saveVersion >= 9' },				// The number of times Scried for snowballs
					{ 'format': 'UInt16', 'key': 'snowballSize', 'cond': 'saveVersion >= 9' },					// The number of uncollected snowballs
					{ 'format': 'Uint32', 'key': 'lastGiftDate', 'cond': 'saveVersion >= 9' },					// Date of most recently earned Gift
					{ 'format': 'Int32', 'key': 'chargedTimer' },												// 
					{ 'format': 'Int32', 'key': 'comboStrike' },												// 
					{ 'format': 'Int32', 'key': 'comboStrikeCont' },											// ??? Whether comboStrike is in contingency autocast?
					{ 'format': 'Int32', 'key': 'goblinTimer' },												// ??? GFD?
					{ 'format': 'Int32', 'key': 'unknown/unk4', 'cond': 'saveVersion >= 11' },												// ???
					...
					{ 'format': 'Int8', 'key': 'season', 'cond': 'saveVersion >= 2' },							// Season (0: None, 1: Thanksgiving, 2: Christmas)
					{ 'format': 'Int8', 'key': 'bFaction' },													// 
					{ 'format': 'Int8', 'key': 'buyMode' },														// Multibuy button mode (0: Buy 1, 1: Buy 10, 2: Buy 100, 3: Buy All)
					{ 'format': 'Int8', 'key': 'excavBuyMode', 'cond': 'saveVersion >= 10' },					// Excavation Multibuy button mode (0: Buy 1, 1: Buy 10, 2: Buy 100, 3: Buy All)
