  ...
  */
  ...
  window.SaveFormat = new function(saveFormat) {
    ...
    var decodeSave = function(save) {
      ...
      // Convert to Array of charCodes [ fancy arrow function! :) ]
  ...
  $(function() {
    ...
    $('#saveInput').on('paste', function(e) {
      var save = '';
      if (e.originalEvent)
        save = e.originalEvent.clipboardData.getData('text');
      if (!save)
        save = $(this).val();
      
      if (save) {
        var decoded = window.decoded = SaveFormat.Decode(save);
        console.log('Decoded Save Format: ', decoded);
        //console.log('Re-encoded Save Format: ', SaveFormat.Encode(decoded));
      }
    }).trigger('paste').trigger('focus');
    
    // Bind Copy button
    $('#doSaveCopy').on('click', function(e) {
      $('#saveInput').val('').trigger('focus');
      var save = $('#saveInput').val();
      window.prompt('Copy to clipboard: Ctrl+C, Enter', save);
    });
  ...
  /**
   * Refactoring: SaveHandler
   * Refactoring of the above save decoding/encoding, and save format parsing/compiling
   * 
   */
  ...
  window.SaveHandler = new function() {
    ...
    // Decode a given save string into a JS object
    var decode = function(save) {
      ...
      for (var formatType in formatTypeChecks) {
        ...
        if (formatTypeCheck)
          return decodeType[formatType].apply(undefined, formatTypeCheck.slice(1).reverse());
      }
    ...
    // Encode a given JS object into a save string
    var encode = function(save, formatType, version) {
      // Set default formatType
      if (!formatType)
        formatType = 'bytes';
      
      // Check if we can match any known save format types and redirect there
      if (formatTypeChecks.hasOwnProperty(formatType))
        encodeType[formatType].apply(undefined, [save].concat([].slice.call(arguments, 2)));
    ...
    // Easily switch between compression method
    var compression = {
      'compressor': pako, 
    ...
    var compression = {
      'compressor': Zlib, 
      'inflate': function(save) { (new Zlib.Inflate(save)).decompress(); }, 
      'deflate': function(save) { (new Zlib.Deflate(save)).compress(); }, 
    };
    */
    ...
    var decodeType = {
      ...
      'bytes': function decodeBytes(save, version) {
        // Base64 Decode
        save = atob(save);
        ...
        // Parse Save Format
        save = SaveFormat.Parse(dataView);
    ...
    var encodeType = {
      ...
      'bytes': function encodeBytes(save, version) {
        // Compile Save Format
        var save = SaveFormat.Compile(save);
    ...
    // Bind the decode endpoint
    this.Decode = decode.bind(undefined);
    
    // Bind the encode endpoint
    this.Encode = encode.bind(undefined);
    
  }();
  
}(window, document, jQuery));

decode = window.SaveFormat.Decode;
encode = window.SaveFormat.Encode;